...
const submit = () => {
    if (!validate()) return;
    // clear server errors if supported
    (form as unknown as any).clearErrors?.();
    form.post('/shipments', { preserveScroll: true });
};
</script>
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-4 p-4">
            <div>
                <h1 class="text-3xl font-bold">Create Shipment</h1>
                <p class="text-muted-foreground">Add a new shipment</p>
            </div>
            ...
            <Card class="max-w-2xl">
                ...
                <form @submit.prevent="submit">
                    ...
                    <CardFooter class="flex justify-between">
                        ...
                        <Button type="submit" :disabled="form.processing">{{ form.processing ? 'Creating...' : 'Create Shipment' }}</Button>
                    </CardFooter>
                </form>
            </Card>
        </div>
